...
## Overview
 ...
 MCP X-Ray is designed as a single, unified security scanning solution for MCP servers, eliminating the need to manage and integrate multiple separate tools. It scans for security issues across multiple dimensions including configuration vulnerabilities, exposed secrets and unsafe tool definitions and implementations.
...
./mcpxray config-scan /path/to/mcp/config.json --analyzer-type token

...
MCP X-Ray is a unified security scanning and penetration testing solution for MCP servers, removing the need to manage and integrate multiple disparate tools. It analyzes MCPs across multiple security dimensions, including configuration vulnerabilities, exposed secrets, and unsafe tool definitions and implementations. In addition, MCP X-Ray performs active penetration testing against MCP servers to uncover real-world, exploitable vulnerabilities.
...
- **Connection Security**:
  ...
  - Detects localhost/loopback addresses that may expose services to remote exploitation
  - Validates OAuth 2.0 configuration including Protected Resource Metadata (PRM) and Authorization Server Metadata (ASMD) discovery
- **Secrets Detection**: Scans configuration files for exposed credentials, API keys, and other sensitive information.
- **Tool Analysis**: Analyzes tool descriptions using the default **Token Analyzer** or advanced **LLM Analyzer** for potential security risks such as
...
Executes security test plans by making actual tool calls against MCP servers. Test plans are generated by LLMs based on tool definitions.

**Detection Capabilities:**
- **Code Execution**: JavaScript injection, SQL injection, arbitrary code execution, sandbox escape
- **SSRF**: Access to internal resources, cloud metadata services (AWS/GCP/Azure), localhost services
- **Path Traversal**: File system access, directory traversal, unauthorized file operations
- **Authorization Bypass**: Cross-tenant access, privilege escalation, access control bypass
- **Input Injection**: SQL injection, command injection, XSS, protocol handler abuse
- **Information Disclosure**: Error messages leaking sensitive data, stack traces, system paths
- **DoS**: Resource exhaustion, timeout handling, memory limits
...
- **SCA (Software Composition Analysis)**: Detects vulnerable dependencies using OSV API
- **SAST (Static Application Security Testing)**: Identifies unsafe command patterns and security anti-patterns in code
- **Secrets Detection**: Scans source code for hardcoded secrets and credentials
